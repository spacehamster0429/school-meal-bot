...
### 급식봇 소개 페이지: https://bots.serika.duckdns.org/school-meal-bot/

## 기능

- `/학교설정 이름:학교명`: 학교를 검색해 사용자 기본 학교로 저장
- `/학교설정`: 저장된 학교 설정 삭제 확인
- `/급식`: 날짜를 생략하면 오늘 급식을 조회하고, `20260521` 또는 `5월 21일` 같은 날짜 입력도 지원
- `/급식 나만보기:켜짐`: 서버 채널에서 결과를 본인에게만 표시
- `/내정보`: 저장된 학교 설정 확인
...
봇은 기능 제공을 위해 로컬 SQLite DB에 Discord 사용자 ID와 선택한 학교의 교육청 코드, 학교 코드, 학교명을 저장합니다. `/내정보`로 저장된 학교 설정을 확인할 수 있고, `/학교설정`을 이름 없이 실행해 삭제에 동의하면 해당 사용자의 저장 정보가 삭제됩니다. DB 파일은 기본적으로 권한 `0600`으로 제한되며 외부 분석이나 원격 텔레메트리를 사용하지 않습니다.
