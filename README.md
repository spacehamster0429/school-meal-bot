...
봇은 기능 제공을 위해 로컬 SQLite DB에 Discord 사용자 ID와 선택한 학교의 교육청 코드, 학교 코드, 학교명을 저장합니다. `/학교설정`을 이름 없이 실행해 삭제에 동의하면 해당 사용자의 저장 정보가 삭제됩니다. DB 파일은 기본적으로 권한 `0600`으로 제한되며 외부 분석이나 원격 텔레메트리를 사용하지 않습니다.
